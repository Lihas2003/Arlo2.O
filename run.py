...
def _ensure_thinking_audio():
    """Ensure the thinking sound effect exists."""
    try:
...
def _validate_startup():
    """Check for essential directories and keys."""
...
if __name__ == "__main__":
    ...
    try:
        # Standard uvicorn execution with loop-prevention logic
        uvicorn.run(
            "app.main:app",
            host="0.0.0.0",
            port=8000,
            reload=True,
            # CRITICAL FIX: Only watch the 'app' directory for changes
            reload_dirs=["app"],
            # CRITICAL FIX: Explicitly ignore the heavy .venv and cache folders
